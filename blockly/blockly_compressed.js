...
goog.style.installSafeStyleSheet = function(a, b) {
    var c = goog.dom.getDomHelper(b),
        d, e = c.getDocument();
    goog.userAgent.IE && e.createStyleSheet ? (d = e.createStyleSheet(), goog.style.setSafeStyleSheet(d, a)) : (e = c.getElementsByTagNameAndClass("HEAD")[0], e || (d = c.getElementsByTagNameAndClass("BODY")[0], e = c.createDom("HEAD"), d.parentNode.insertBefore(e, d)), d = c.createDom("STYLE"), goog.style.setSafeStyleSheet(d, a), c.appendChild(e, d));
    return d
};
...
goog.ui.PaletteRenderer.prototype.createDom = function(a) {
    ...
    a = a.getDomHelper().createDom("DIV", b ? b.join(" ") : null, this.createGrid(a.getContent(), a.getSize(), a.getDomHelper()));
...
Blockly.Workspace.prototype.removeChangeListener = function(a) {
    a = this.listeners_.indexOf(a); - 1 != a && this.listeners_.splice(a, 1)
};
...
Blockly.Icon.prototype.iconClick_ = function(a) {
    Blockly.dragMode_ != Blockly.DRAG_FREE && (this.block_.isInFlyout || Blockly.isRightButton(a) || this.setVisible(!this.isVisible()))
};
...
Blockly.Comment.prototype.createEditor_ = function() {
    ...
    Blockly.bindEvent_(b, "mouseup", this, this.textareaFocus_);
    Blockly.bindEvent_(b, "wheel", this, function(a) {
        a.stopPropagation()
    });
    Blockly.bindEvent_(b, "change", this, function(a) {
        this.text_ != b.value && (Blockly.Events.fire(new Blockly.Events.Change(this.block_, "comment", null, this.text_, b.value)), this.text_ = b.value)
    });
    setTimeout(function() {
        b.focus()
    }, 0);
...
Blockly.Comment.prototype.setText = function(a) {
    this.text_ != a && (Blockly.Events.fire(new Blockly.Events.Change(this.block_, "comment", null, this.text_, a)), this.text_ = a);
...
Blockly.Connection.prototype.connect_ = function(a) {
    ...
    if (b.isConnected()) {
        ...
        if (e.isShadow()) f = Blockly.Xml.blockToDom(e), e.dispose(), e = null;
            ...
            for (g = d; g.nextConnection;)
                if (g.nextConnection.isConnected()) g = g.getNextBlock();
                else {
        ...
        if (e && (b.disconnect(), Blockly.Events.recordUndo)) {
            var h = Blockly.Events.getGroup();
            setTimeout(function() {
                e.workspace && !e.getParent() && (Blockly.Events.setGroup(h), e.outputConnection ? e.outputConnection.bumpAwayFrom_(b) :
    ...
    var k;
    Blockly.Events.isEnabled() && (k = new Blockly.Events.Move(d));
    Blockly.Connection.connectReciprocally_(b, a);
    d.setParent(c);
    k && (k.recordNew(), Blockly.Events.fire(k))
};
...
Blockly.Connection.prototype.canConnectWithReason_ = function(a) {
    if (a) {
        if (this.sourceBlock_ && a.getSourceBlock() == this.sourceBlock_) return Blockly.Connection.REASON_SELF_CONNECTION;
        if (a.type != Blockly.OPPOSITE_TYPE[this.type]) return Blockly.Connection.REASON_WRONG_TYPE;
        if (this.sourceBlock_ && a.getSourceBlock() && this.sourceBlock_.workspace !== a.getSourceBlock().workspace) return Blockly.Connection.REASON_DIFFERENT_WORKSPACES;
        if (!this.checkType_(a)) return Blockly.Connection.REASON_CHECKS_FAILED
    } else return Blockly.Connection.REASON_TARGET_NULL;
...
Blockly.Connection.prototype.isConnectionAllowed = function(a) {
    var b = this.canConnectWithReason_(a);
    if (b != Blockly.Connection.CAN_CONNECT && b != Blockly.Connection.REASON_MUST_DISCONNECT) return !1;
    if (a.type == Blockly.OUTPUT_VALUE || a.type == Blockly.PREVIOUS_STATEMENT)
        if (a.isConnected() || this.isConnected()) return !1;
    return a.type == Blockly.INPUT_VALUE && a.isConnected() && !a.targetBlock().isMovable() && !a.targetBlock().isShadow() || this.type == Blockly.PREVIOUS_STATEMENT && a.isConnected() && !this.sourceBlock_.nextConnection ||
...
Blockly.Connection.prototype.disconnect = function() {
    ...
    var b, c;
    this.isSuperior() ? (b = this.sourceBlock_, c = a.getSourceBlock(), a = this) : (b = a.getSourceBlock(), c = this.sourceBlock_);
...
Blockly.Connection.prototype.respawnShadow_ = function() {
    ...
    if (a.workspace && b && Blockly.Events.recordUndo) {
        a = Blockly.Xml.domToBlock(b, a.workspace);
        if (a.outputConnection) this.connect(a.outputConnection);
...
Blockly.Connection.prototype.setCheck = function(a) {
    a ? (goog.isArray(a) || (a = [a]), this.check_ = a, this.isConnected() && !this.checkType_(this.targetConnection) && ((this.isSuperior() ? this.targetBlock() : this.sourceBlock_).unplug(), this.sourceBlock_.bumpNeighbours_())) : this.check_ = null;
    return this
};
...
Blockly.Field.prototype.init = function() {
    this.fieldGroup_ || (this.fieldGroup_ = Blockly.createSvgElement("g", {}, null), this.visible_ || (this.fieldGroup_.style.display = "none"), this.borderRect_ = Blockly.createSvgElement("rect", {
            rx: 4,
            ry: 4,
...
Blockly.Field.prototype.setText = function(a) {
    null !== a && (a = String(a), a !== this.text_ && (this.text_ = a, this.updateTextNode_(), this.sourceBlock_ && this.sourceBlock_.rendered && (this.sourceBlock_.render(), this.sourceBlock_.bumpNeighbours_())))
};
...
Blockly.Field.prototype.setValue = function(a) {
    if (null !== a) {
        var b = this.getValue();
        b != a && (this.sourceBlock_ && Blockly.Events.isEnabled() && Blockly.Events.fire(new Blockly.Events.Change(this.sourceBlock_, "field", this.name, b, a)), this.setText(a))
    }
};
Blockly.Field.prototype.onMouseUp_ = function(a) {
    if (!goog.userAgent.IPHONE && !goog.userAgent.IPAD || goog.userAgent.isVersionOrHigher("537.51.2") || 0 === a.layerX || 0 === a.layerY) Blockly.isRightButton(a) || Blockly.dragMode_ != Blockly.DRAG_FREE && this.sourceBlock_.isEditable() && this.showEditor_()
};
...
Blockly.Tooltip.onMouseMove_ = function(a) {
    if (Blockly.Tooltip.element_ && Blockly.Tooltip.element_.tooltip && Blockly.dragMode_ == Blockly.DRAG_NONE && !Blockly.WidgetDiv.isVisible())
        ...
        } else Blockly.Tooltip.poisonedElement_ != Blockly.Tooltip.element_ && (clearTimeout(Blockly.Tooltip.showPid_), Blockly.Tooltip.lastX_ = a.pageX, Blockly.Tooltip.lastY_ = a.pageY,
...
Blockly.FieldLabel.prototype.init = function() {
    ...
    }, null), this.class_ && Blockly.addClass_(this.textElement_, this.class_), this.visible_ || (this.textElement_.style.display = "none"), this.sourceBlock_.getSvgRoot().appendChild(this.textElement_), this.textElement_.tooltip = this.sourceBlock_, Blockly.Tooltip.bindMouseEvents(this.textElement_), this.updateTextNode_())
};
...
Blockly.Options = function(a) {
    ...
    var m = a.toolboxPosition,
        ...
        l ? Blockly.TOOLBOX_AT_RIGHT : Blockly.TOOLBOX_AT_LEFT,
        q = a.scrollbars;
    void 0 === q && (q = d);
    var n = a.css;
    void 0 === n && (n = !0);
...
Blockly.Options.prototype.setMetrics = function(a) {};
Blockly.Options.prototype.getMetrics = function() {
    return null
};
...
Blockly.ScrollbarPair.prototype.set = function(a, b) {
    var c = {},
        ...
        f = parseFloat(this.hScroll.svgBackground_.getAttribute("width")),
        g = parseFloat(this.vScroll.svgBackground_.getAttribute("height"));
    c.x = this.getRatio_(d, f);
    c.y = this.getRatio_(e, g);
    this.workspace_.setMetrics(c);
    this.hScroll.svgKnob_.setAttribute("x", d);
    this.vScroll.svgKnob_.setAttribute("y", e)
};
...
Blockly.Scrollbar.prototype.resizeHorizontal_ = function(a) {
    ...
    var c = (a.viewLeft - a.contentLeft) * this.ratio_;
    this.svgKnob_.setAttribute("width", Math.max(0, a.viewWidth * this.ratio_));
    this.xCoordinate = a.absoluteLeft + .5;
    this.pair_ && this.workspace_.RTL && (this.xCoordinate += Blockly.Scrollbar.scrollbarThickness);
    ...
    this.svgKnob_.setAttribute("x", this.constrainKnob_(c))
};
Blockly.Scrollbar.prototype.resizeVertical_ = function(a) {
    ...
    if (-Infinity === this.ratio_ || Infinity === this.ratio_ || isNaN(this.ratio_)) this.ratio_ = 0;
    var c = (a.viewTop - a.contentTop) * this.ratio_;
    this.svgKnob_.setAttribute("height", Math.max(0, a.viewHeight * this.ratio_));
    this.xCoordinate = a.absoluteLeft + .5;
    this.workspace_.RTL || (this.xCoordinate += a.viewWidth - Blockly.Scrollbar.scrollbarThickness -
        1);
    this.yCoordinate = a.absoluteTop + .5;
    this.svgGroup_.setAttribute("transform", "translate(" + this.xCoordinate + "," + this.yCoordinate + ")");
    this.svgBackground_.setAttribute("height", Math.max(0, b));
    this.svgKnob_.setAttribute("y", this.constrainKnob_(c))
};
...
Blockly.Scrollbar.prototype.onMouseDownBar_ = function(a) {
    this.onMouseUpKnob_();
    if (Blockly.isRightButton(a)) a.stopPropagation();
    else {
        var b = Blockly.mouseToSvg(a, this.workspace_.getParentSvg()),
            b = this.horizontal_ ? b.x : b.y,
            c = Blockly.getSvgXY_(this.svgKnob_, this.workspace_),
            ...
            f = .95 * d;
        b <= c ? e -= f : b >= c + d && (e += f);
        this.svgKnob_.setAttribute(this.horizontal_ ?
            "x" : "y", this.constrainKnob_(e));
        this.onScroll_();
        a.stopPropagation();
        a.preventDefault()
...
Blockly.Scrollbar.prototype.set = function(a) {
    a = this.constrainKnob_(a * this.ratio_);
    this.svgKnob_.setAttribute(this.horizontal_ ? "x" : "y", a);
    this.onScroll_()
};
...
Blockly.Trashcan.prototype.click = function() {
    ...
    Math.sqrt(a * a + b * b) > Blockly.DRAG_RADIUS
};
Blockly.Xml = {};
Blockly.Xml.workspaceToDom = function(a) {
    var b = goog.dom.createDom("xml");
    a = a.getTopBlocks(!0);
    for (var c = 0, d; d = a[c]; c++) b.appendChild(Blockly.Xml.blockToDomWithXY(d));
    return b
};
Blockly.Xml.blockToDomWithXY = function(a) {
    var b;
    a.workspace.RTL && (b = a.workspace.getWidth());
    var c = Blockly.Xml.blockToDom(a),
        d = a.getRelativeToSurfaceXY();
    c.setAttribute("x", Math.round(a.workspace.RTL ? b - d.x : d.x));
    c.setAttribute("y", Math.round(d.y));
    return c
};
Blockly.Xml.blockToDom = function(a) {
    ...
    for (var c = 0, d; d = a.inputList[c]; c++)
        ...
        c), "object" == typeof a.comment && (c.setAttribute("pinned", a.comment.isVisible()), d = a.comment.getBubbleSize(), c.setAttribute("h", d.height), c.setAttribute("w", d.width)), b.appendChild(c);
    a.data && (c = goog.dom.createDom("data", null, a.data), b.appendChild(c));
    for (c = 0; d = a.inputList[c]; c++) {
        var h;
        f = !0;
        d.type != Blockly.DUMMY_INPUT && (g = d.connection.targetBlock(), d.type == Blockly.INPUT_VALUE ? h = goog.dom.createDom("value") : d.type == Blockly.NEXT_STATEMENT && (h = goog.dom.createDom("statement")), e = d.connection.getShadowDom(),
            !e || g && g.isShadow() || h.appendChild(Blockly.Xml.cloneShadow_(e)), g && (h.appendChild(Blockly.Xml.blockToDom(g)), f = !1), h.setAttribute("name", d.name), f || b.appendChild(h))
    }
    ...
    if (c = a.getNextBlock()) h = goog.dom.createDom("next", null, Blockly.Xml.blockToDom(c)), b.appendChild(h);
...
Blockly.Xml.domToWorkspace = function(a, b) {
    ...
    for (var f = 0; f < c; f++) {
        ...
        if ("block" == h || "shadow" == h) {
            var h = Blockly.Xml.domToBlock(g, b),
                k = parseInt(g.getAttribute("x"), 10),
                g = parseInt(g.getAttribute("y"),
                    10);
...
Blockly.Xml.domToBlock = function(a, b) {
    ...
    if (b.rendered) {
        ...
        for (var c = d.getDescendants(), e = c.length - 1; 0 <= e; e--) c[e].initSvg();
        for (e = c.length - 1; 0 <= e; e--) c[e].render(!1);
        setTimeout(function() {
            d.workspace && d.setConnectionsHidden(!1)
        }, 1);
        d.updateDisabled();
        Blockly.asyncSvgResize(b)
    }
...
Blockly.Xml.domToBlockHeadless_ = function(a, b) {
    ...
    for (var e = a.getAttribute("id"), c = b.newBlock(d, e), f = null, e = 0, g; g = a.childNodes[e]; e++)
        if (3 != g.nodeType) {
            ...
            switch (g.nodeName.toLowerCase()) {
                ...
                case "next":
                    ...
                    if (f) {
                        ...
                        if (c.nextConnection.isConnected()) throw "Next statement is already connected.";
        ...
        }(d = a.getAttribute("inline")) && c.setInputsInline("true" == d);
    (d = a.getAttribute("disabled")) && c.setDisabled("true" == d);
    (d = a.getAttribute("deletable")) &&
    c.setDeletable("true" == d);
    (d = a.getAttribute("movable")) && c.setMovable("true" == d);
    (d = a.getAttribute("editable")) && c.setEditable("true" == d);
    (d = a.getAttribute("collapsed")) && c.setCollapsed("true" == d);
    "shadow" == a.nodeName.toLowerCase() && c.setShadow(!0);
...
Blockly.ZoomControls.prototype.createDom = function() {
    ...
    Blockly.bindEvent_(b, "mousedown", null, function(b) {
        a.setScale(1);
        a.scrollCenter();
        b.stopPropagation();
        b.preventDefault()
    });
    Blockly.bindEvent_(e, "mousedown", null, function(b) {
        a.zoomCenter(1);
        b.stopPropagation();
        b.preventDefault()
    });
...
Blockly.WorkspaceSvg.prototype.highlightBlock = function(a) {
    this.traceOn_ && Blockly.dragMode_ != Blockly.DRAG_NONE && this.traceOn(!1);
    if (this.traceOn_) {
        var b = null;
        ...
        if (!isNaN(c) && !isNaN(a)) {
            ...
            do {
                for (var d = !1, e = this.getAllBlocks(), f = 0, g; g = e[f]; f++)
                    ...
                    for (e = b.getConnections_(!1),
                        f = 0; g = e[f]; f++)
                        if (g.closest(Blockly.SNAP_RADIUS, new goog.math.Coordinate(c, a)).connection) {
                            d = !0;
                            break
...
Blockly.WorkspaceSvg.prototype.startDrag = function(a, b) {
    var c = Blockly.mouseToSvg(a, this.getParentSvg());
    c.x /= this.scale;
    c.y /= this.scale;
    ...
    if (!this.options.readOnly && !this.isFlyout) {
        ...
        if (this.options.collapse) {
            for (var h = g = !1, k = 0; k < e.length; k++)
                for (var l = e[k]; l;) l.isCollapsed() ? g = !0 : h = !0, l = l.getNextBlock();
            var p = function(a) {
        ...
        for (var m = [], k = 0; k < e.length; k++) b(e[k]);
        g = {
            text: 1 == m.length ? Blockly.Msg.DELETE_BLOCK : Blockly.Msg.DELETE_X_BLOCKS.replace("%1", String(m.length)),
            enabled: 0 < m.length,
            callback: function() {
                (2 > m.length || window.confirm(Blockly.Msg.DELETE_ALL_BLOCKS.replace("%1", String(m.length)))) && c()
            }
        };
        d.push(g);
...
Blockly.WorkspaceSvg.prototype.updateGridPattern_ = function() {
    if (this.options.gridPattern) {
        ...
        var a = Math.floor(this.options.gridOptions.spacing / 2) + .5,
            b = a - this.options.gridOptions.length / 2,
...
Blockly.Mutator.prototype.resizeBubble_ = function() {
    var a = 2 * Blockly.Bubble.BORDER_WIDTH,
        ...
        c;
    c = this.block_.RTL ? -b.x : b.width + b.x;
    b = b.height + 3 * a;
    if (this.workspace_.flyout_) var d = this.workspace_.flyout_.getMetrics_(),
...
Blockly.Block = function(a, b, c) {
    ...
    goog.isFunction(this.onchange) && (this.onchangeWrapper_ = this.onchange.bind(this), this.workspace.addChangeListener(this.onchangeWrapper_))
};
...
Blockly.Block.prototype.dispose = function(a) {
    ...
    a = 0;
    for (var b; b = this.inputList[a]; a++) b.dispose();
...
Blockly.Block.prototype.renameVar = function(a, b) {
    for (var c = 0, d; d = this.inputList[c]; c++)
        for (var e = 0, f; f = d.fieldRow[e]; e++) f instanceof Blockly.FieldVariable && Blockly.Names.equals(a, f.getValue()) && f.setValue(b)
};
Blockly.Block.prototype.getInstances = function(a) {
    for (var b = [], c = 0, d; d = this.inputList[c]; c++)
        for (var e = 0, f; f = d.fieldRow[e]; e++) f instanceof Blockly.FieldInstance && (f = a ? f.getInstanceTypeValue(a) : f.getValue()) && b.push(f);
    return b
};
Blockly.Block.prototype.renameInstance = function(a, b, c) {
    for (var d = 0, e; e = this.inputList[d]; d++)
        for (var f = 0, g; g = e.fieldRow[f]; f++)
            if (g instanceof Blockly.FieldInstance) {
                var h = g.getInstanceTypeValue(c);
                h && Blockly.Names.equals(a, h) && g.setValue(b)
            }
};
...
Blockly.Block.prototype.setInputsInline = function(a) {
    this.inputsInline != a && (Blockly.Events.fire(new Blockly.Events.Change(this, "inline", null, this.inputsInline, a)), this.inputsInline = a)
};
...
Blockly.Block.prototype.setDisabled = function(a) {
    this.disabled != a && (Blockly.Events.fire(new Blockly.Events.Change(this, "disabled", null, this.disabled, a)), this.disabled = a)
};
...
Blockly.Block.prototype.setCollapsed = function(a) {
    this.collapsed_ != a && (Blockly.Events.fire(new Blockly.Events.Change(this, "collapsed", null, this.collapsed_, a)), this.collapsed_ = a)
};
        ...
        if (e = a[g], "string" == typeof e) b.push([e, void 0]);
            ...
            do switch (h = !1, e.type) {
                ...
                case "field_image":
                    ...
                    break;
                case "field_number":
                    f = new Blockly.FieldNumber(e.text);
                    break;
                case "field_date":
...
Blockly.Block.prototype.setCommentText = function(a) {
    this.comment != a && (Blockly.Events.fire(new Blockly.Events.Change(this, "comment", null, this.comment, a || "")), this.comment = a)
};
...
Blockly.RenderedConnection.prototype.highlight = function() {
    var a;
    a = this.type == Blockly.INPUT_VALUE || this.type == Blockly.OUTPUT_VALUE ? "m 0,0 " + Blockly.BlockSvg.TAB_PATH_DOWN + " v 5" : "m -20,0 h 5 " + Blockly.BlockSvg.NOTCH_PATH_LEFT + " h 5";
...
Blockly.RenderedConnection.prototype.respawnShadow_ = function() {
    var a = this.getSourceBlock(),
        b = this.getShadowDom();
    if (a.workspace && b && Blockly.Events.recordUndo) {
        b = Blockly.RenderedConnection.superClass_.respawnShadow_.call(this);
...
Blockly.BlockSvg.prototype.select = function() {
    if (this.isShadow() && this.getParent()) this.getParent().select();
        ...
        var a = null;
        Blockly.selected && (a = Blockly.selected.id, Blockly.Events.disable(), Blockly.selected.unselect(), Blockly.Events.enable());
...
Blockly.BlockSvg.prototype.getBoundingRectangle = function() {
    var a = this.getRelativeToSurfaceXY(this),
        ...
        d;
    this.RTL ? (d = new goog.math.Coordinate(a.x - (c.width - b), a.y), a = new goog.math.Coordinate(a.x + b, a.y + c.height)) : (d = new goog.math.Coordinate(a.x - b, a.y), a = new goog.math.Coordinate(a.x + c.width - b, a.y + c.height));
    return {
        topLeft: d,
        bottomRight: a
    }
...
Blockly.BlockSvg.prototype.onMouseDown_ = function(a) {
    if (!this.workspace.options.readOnly)
        if (this.isInFlyout) a.stopPropagation();
        else {
...
Blockly.BlockSvg.prototype.onMouseUp_ = function(a) {
    ...
    Blockly.selected && Blockly.highlightedConnection_ ? (Blockly.localConnection_.connect(Blockly.highlightedConnection_), this.rendered && (Blockly.localConnection_.isSuperior() ? Blockly.highlightedConnection_ : Blockly.localConnection_).getSourceBlock().connectionUiEffect(), this.workspace.trashcan && this.workspace.trashcan.close()) :
        !this.getParent() && Blockly.selected.isDeletable() && this.workspace.isDeleteArea(a) && ((a = this.workspace.trashcan) && goog.Timer.callOnce(a.close, 100, a), Blockly.selected.dispose(!1, !0), Blockly.asyncSvgResize(this.workspace));
...
Blockly.BlockSvg.prototype.showContextMenu_ = function(a) {
    if (!this.workspace.options.readOnly && this.contextMenu) {
        ...
        if (this.isDeletable() && this.isMovable() && !b.isInFlyout) {
            ...
            d = {
                text: 1 == d ?
                    Blockly.Msg.DELETE_BLOCK : Blockly.Msg.DELETE_X_BLOCKS.replace("%1", String(d)),
                enabled: !0,
                callback: function() {
                    Blockly.Events.setGroup(!0);
                    b.dispose(!0, !0);
...
Blockly.BlockSvg.prototype.setDragging_ = function(a) {
    ...
    } else this.removeDragging(), Blockly.draggingConnections_ = [];
    for (b = 0; b < this.childBlocks_.length; b++) this.childBlocks_[b].setDragging_(a)
};
Blockly.BlockSvg.prototype.onMouseMove_ = function(a) {
    if ("mousemove" == a.type && 1 >= a.clientX && 0 == a.clientY && 0 == a.button) a.stopPropagation();
        ...
        if (Blockly.dragMode_ ==
            ...
            for (c = 0; c < this.draggedBubbles_.length; c++) d = this.draggedBubbles_[c], d.bubble.setIconLocation(goog.math.Coordinate.sum(d, b));
            d = this.getConnections_(!1);
            (c = this.lastConnectionInStack_()) && c != this.nextConnection && d.push(c);
            for (var e = null, f = null, g = Blockly.SNAP_RADIUS, c = 0; c < d.length; c++) {
                var h = d[c],
                    k = h.closest(g, b);
                k.connection &&
                    (e = k.connection, f = h, g = k.radius)
            }
            Blockly.highlightedConnection_ && Blockly.highlightedConnection_ != e && (Blockly.highlightedConnection_.unhighlight(), Blockly.highlightedConnection_ = null, Blockly.localConnection_ = null);
            e && e != Blockly.highlightedConnection_ && (e.highlight(), Blockly.highlightedConnection_ = e, Blockly.localConnection_ = f);
...
Blockly.BlockSvg.prototype.updateColour = function() {
    if (!this.disabled) {
        ...
        for (a = 0; c = this.inputList[a]; a++)
            for (var b = 0, d; d = c.fieldRow[b]; b++) d.setText(null)
    }
};
...
Blockly.BlockSvg.prototype.render = function(a) {
    ...
    !1 !== a && ((a = this.getParent()) ? a.render(!0) : Blockly.asyncSvgResize(this.workspace));
    Blockly.Field.stopCache()
};
...
Blockly.BlockSvg.prototype.renderCompute_ = function(a) {
    ...
    for (var d = 0, e = 0, f = !1, g = !1, h = !1, k = void 0, l = this.getInputsInline() && !this.isCollapsed(), p = 0, m; m = b[p]; p++)
        if (m.isVisible()) {
            var q;
            l && k && k != Blockly.NEXT_STATEMENT && m.type != Blockly.NEXT_STATEMENT ? q = c[c.length - 1] : (k = m.type, q = [], q.type = l && m.type !=
                Blockly.NEXT_STATEMENT ? Blockly.BlockSvg.INLINE : m.type, q.height = 0, c.push(q));
            q.push(m);
            ...
            l || p != b.length - 1 ? !l && m.type == Blockly.INPUT_VALUE && b[p + 1] && b[p + 1].type == Blockly.NEXT_STATEMENT &&
                m.renderHeight-- : m.renderHeight--;
            ...
            q.type != Blockly.BlockSvg.INLINE && (q.type == Blockly.NEXT_STATEMENT ? (g = !0, e = Math.max(e, m.fieldWidth)) :
                (q.type == Blockly.INPUT_VALUE ? f = !0 : q.type == Blockly.DUMMY_INPUT && (h = !0), d = Math.max(d, m.fieldWidth)))
        } for (a = 0; q = c[a]; a++)
    ...
    f ? c.rightEdge = Math.max(c.rightEdge, d + 2 * Blockly.BlockSvg.SEP_SPACE_X + Blockly.BlockSvg.TAB_WIDTH) :
        h && (c.rightEdge = Math.max(c.rightEdge, d + 2 * Blockly.BlockSvg.SEP_SPACE_X));
    c.hasValue = f;
    c.hasStatement = g;
    c.hasDummy = h;
    return c
...
Blockly.BlockSvg.prototype.renderDrawBottom_ = function(a, b, c, d) {
    this.height += d + 1;
    this.nextConnection && (a.push("H", Blockly.BlockSvg.NOTCH_WIDTH + (this.RTL ? .5 : -.5) + " " + Blockly.BlockSvg.NOTCH_PATH_RIGHT), this.nextConnection.moveTo(this.RTL ? c.x - Blockly.BlockSvg.NOTCH_WIDTH : c.x + Blockly.BlockSvg.NOTCH_WIDTH, c.y + d + 1), this.nextConnection.isConnected() && this.nextConnection.tighten_(), this.height += 4);
...
Blockly.Events.filter = function(a, b) {
    ...
    for (var d = 0, e; e = c[d]; d++)
        for (var f = d + 1, g; g = c[f]; f++) e.type == g.type && e.blockId == g.blockId && e.workspaceId == g.workspaceId && (e.type == Blockly.Events.MOVE ? (e.newParentId = g.newParentId, e.newInputName = g.newInputName, e.newCoordinate = g.newCoordinate, c.splice(f, 1), f--) : e.type == Blockly.Events.CHANGE && e.element == g.element && e.name == g.name ? (e.newValue = g.newValue, c.splice(f, 1), f--) : e.type != Blockly.Events.UI || "click" != g.element || "commentOpen" !=
            e.element && "mutatorOpen" != e.element && "warningOpen" != e.element || (e.newValue = g.newValue, c.splice(f, 1), f--));
    for (d = c.length - 1; 0 <= d; d--) c[d].isNull() && c.splice(d, 1);
    b || c.reverse();
    for (d = 1; e = c[d]; d++) e.type == Blockly.Events.CHANGE && "mutation" == e.element && c.unshift(c.splice(d, 1)[0]);
    return c
};
...
Blockly.Events.fromJson = function(a, b) {
    ...
    switch (a.type) {
        case Blockly.Events.CREATE:
            c = new Blockly.Events.Create(null);
            break;
...
Blockly.Events.Create = function(a) {
    a && (Blockly.Events.Create.superClass_.constructor.call(this, a), this.xml = Blockly.Xml.blockToDomWithXY(a), this.ids = Blockly.Events.getDescendantIds_(a))
};
...
Blockly.Events.Create.prototype.run = function(a) {
    ...
    if (a) a = goog.dom.createDom("xml"), a.appendChild(this.xml), Blockly.Xml.domToWorkspace(a, b);
        ...
        for (var c; c = this.ids[a]; a++) {
            ...
            d ? d.dispose(!1, !0) : c == this.blockId && console.warn("Can't uncreate non-existant block: " + c)
        }
    }
};
...
Blockly.Events.Delete.prototype.run = function(a) {
    ...
    if (a) {
        ...
        for (var c; c = this.ids[a]; a++) {
            var d = b.getBlockById(c);
            d ? d.dispose(!1, !0) : c == this.blockId && console.warn("Can't delete non-existant block: " + c)
        }
...
Blockly.Events.Change.prototype.run = function(a) {
    ...
    if (b) switch (b.mutator && b.mutator.setVisible(!1), a = a ? this.newValue : this.oldValue, this.element) {
        case "field":
            (b = b.getField(this.name)) ? b.setValue(a): console.warn("Can't set non-existant field: " + this.name);
            break;
        case "comment":
...
Blockly.Events.Move.prototype.run = function(a) {
    ...
    if (c) {
        ...
        if (a) e = c.getRelativeToSurfaceXY(), c.moveBy(a.x - e.x, a.y - e.y);
            ...
            var c = c.outputConnection || c.previousConnection,
                g;
            if (e) {
                if (b = f.getInput(e)) g = b.connection
...
Blockly.FieldTextInput.prototype.showEditor_ = function(a) {
    ...
    if (!a && (goog.userAgent.MOBILE || goog.userAgent.ANDROID || goog.userAgent.IPAD)) {
        a = Blockly.prompt(Blockly.Msg.CHANGE_VALUE_TITLE, this.text_);
        if (this.sourceBlock_ && this.validator_) {
            var b = this.validator_(a);
            void 0 !== b && (a = b)
        }
        ...
        this.resizeEditor_();
        a || (c.focus(), c.select());
        c.onKeyDownWrapper_ = Blockly.bindEvent_(c, "keydown", this, this.onHtmlInputKeyDown_);
        c.onKeyUpWrapper_ = Blockly.bindEvent_(c, "keyup", this, this.onHtmlInputChange_);
        c.onKeyPressWrapper_ = Blockly.bindEvent_(c, "keypress",
...
Blockly.FieldTextInput.prototype.validate_ = function() {
    ...
    var b = Blockly.FieldTextInput.htmlInput_;
    this.sourceBlock_ && this.validator_ && (a = this.validator_(b.value));
...
Blockly.FieldTextInput.prototype.widgetDispose_ = function() {
    ...
    return function() {
        var b = Blockly.FieldTextInput.htmlInput_,
            c = b.value;
        if (a.sourceBlock_ && a.validator_) {
            var d = a.validator_(c);
...
Blockly.FieldAngle.prototype.setValidator = function(a) {
    Blockly.FieldAngle.superClass_.setValidator.call(this, a ? function(b) {
        var c = a.call(this, b);
        if (null === c) var d = c;
...
Blockly.FieldAngle.prototype.onMouseMove = function(a) {
    ...
    b = Math.atan(-a / c);
    isNaN(b) || (b = goog.math.toDegrees(b), 0 > c ? b += 180 : 0 < a && (b += 360), b = Blockly.FieldAngle.CLOCKWISE ? Blockly.FieldAngle.OFFSET + 360 - b : b - Blockly.FieldAngle.OFFSET, Blockly.FieldAngle.ROUND && (b = Math.round(b / Blockly.FieldAngle.ROUND) * Blockly.FieldAngle.ROUND), b = Blockly.FieldAngle.angleValidator(b), Blockly.FieldTextInput.htmlInput_.value =
...
Blockly.FieldAngle.angleValidator = function(a) {
    ...
    null !== a && (a %= 360, 0 > a && (a += 360), a > Blockly.FieldAngle.WRAP && (a -= 360), a = String(a));
    return a
};
...
Blockly.FieldCheckbox.prototype.setValue = function(a) {
    a = "TRUE" == a;
    this.state_ !== a && (this.sourceBlock_ && Blockly.Events.isEnabled() && Blockly.Events.fire(new Blockly.Events.Change(this.sourceBlock_, "field", this.name, this.state_, a)), this.state_ = a, this.checkElement_ && (this.checkElement_.style.display = a ? "block" : "none"))
};
...
Blockly.FieldColour.prototype.showEditor_ = function() {
    ...
    Blockly.FieldColour.changeEventKey_ = goog.events.listen(a, goog.ui.ColorPicker.EventType.CHANGE, function(a) {
        ...
        if (g.sourceBlock_ && g.validator_) {
            var b = g.validator_(a);
            void 0 !==
                b && (a = b)
        }
        null !== a && g.setValue(a)
...
Blockly.Instances.allInstancesOf = function(a, b) {
    var c;
    if (b.getAllBlocks) c = b.getAllBlocks();
    else throw "Not a valid Workspace: " + b;
    for (var d = Object.create(null), e = 0; e < c.length; e++)
        for (var f = c[e].getInstances(a), g = 0; g < f.length; g++) {
            var h = f[g];
            h && (d[h.toLowerCase()] = h)
        }
    c = [];
    for (var k in d) c.push(d[k]);
    return c
};
Blockly.Instances.allInstances = function(a) {
    return Blockly.Instances.allInstancesOf(void 0, a)
};
Blockly.Instances.getAnyInstanceOf = function(a, b) {
    var c;
    if (b.getAllBlocks) c = b.getAllBlocks();
    else throw "Not a valid Workspace: " + b;
    for (var d = 0; d < c.length; d++) {
        var e = c[d].getInstances(a);
        if (e.length) return e[0]
    }
};
Blockly.Instances.isInstancePresent = function(a, b, c) {
    var d;
    if (c.workspace && c.workspace.getAllBlocks) d = c.workspace.getAllBlocks();
    else throw "Not a valid block: " + c;
    for (var e = 0; e < d.length; e++)
        for (var f = d[e].getInstances(b), g = 0; g < f.length; g++)
            if (f[g] === a && d[e] !== c) return !0;
    return !1
};
Blockly.Instances.renameInstance = function(a, b, c, d) {
    Blockly.Events.setGroup(!0);
    d = d.getAllBlocks();
    for (var e = 0; e < d.length; e++) d[e].renameInstance(a, b, c);
    Blockly.Events.setGroup(!1)
};
Blockly.Instances.generateUniqueName = function(a) {
    a = Blockly.Variables.allVariables(a).concat(Blockly.Instances.allInstances(a));
    var b = "";
    if (a.length)
...
Blockly.Instances.convertToUniqueName = function(a, b) {
    var c = Blockly.Variables.allVariables(b).concat(Blockly.Instances.allInstances(b));
    return Blockly.Instances.appendToName_(a, c)
};
Blockly.Instances.convertToUniqueNameBlock = function(a, b) {
    var c;
    if (b.workspace) c = b.workspace.getAllBlocks();
    else throw "Not a valid Workspace: " + workspace;
    for (var d = Object.create(null), e = 0; e < c.length; e++)
        if (c[e] !== b)
            for (var f = c[e].getInstances(), g = 0; g < f.length; g++) {
                var h = f[g];
                h && (d[h.toLowerCase()] = h)
            }
    c = [];
    for (var k in d) c.push(d[k]);
    d = Blockly.Variables.allVariables(b.workspace).concat(c);
    return Blockly.Instances.appendToName_(a, d)
};
Blockly.Instances.appendToName_ = function(a, b) {
    if (a) {
        var c = a,
            d = 2;
        a.match(/_\d+$/) && (c = a.match(/\d+$/)[0], a = a.slice(0, -1 * c.length - 1), d = parseInt(c, 10) + 1, c = a + "_" + d);
        for (; - 1 !== b.indexOf(c);) c = a + "_" + d++;
        return c
    }
...
Blockly.genUid = function() {
    for (var a = Blockly.genUid.soup_.length, b = [], c = 0; 20 > c; c++) b[c] = Blockly.genUid.soup_.charAt(Math.random() * a);
    return b.join("")
};
Blockly.genUid.soup_ = "!#%()*+,-./:;=?@[]^_`{|}~ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
Blockly.prompt = function(a, b, c) {
    if (void 0 === c) return window.prompt(a, b);
    "function" == typeof c ? c(window.prompt(a, b)) : console.log("Blocky prompt callback needs to be a callable function.")
};
Blockly.FieldInstance = function(a, b, c, d, e, f, g) {
    Blockly.FieldInstance.superClass_.constructor.call(this, this.dropdownCreate, g);
    this.instanceType_ = a;
    this.setValue(b);
    this.uniqueName_ = !0 === c;
    this.lockNew_ = !0 === d;
    this.lockRename_ = !0 === e;
    this.editDropdownData = f instanceof Function ? f : null
};
goog.inherits(Blockly.FieldInstance, Blockly.FieldDropdown);
Blockly.FieldInstance.prototype.setValidator = function(a) {
    Blockly.FieldInstance.superClass_.setValidator.call(this, a ? function(b) {
        var c = a.call(this, b);
        if (null === c) var d = c;
        else void 0 === c && (c = b), d = Blockly.FieldInstance.dropdownChange.call(this, c), void 0 === d && (d = c);
        return d === b ? void 0 : d
    } : Blockly.FieldInstance.dropdownChange)
};
Blockly.FieldInstance.prototype.init = function() {
    if (!this.fieldGroup_) {
        Blockly.FieldInstance.superClass_.init.call(this);
        var a = this.sourceBlock_.isInFlyout ? this.sourceBlock_.workspace.targetWorkspace : this.sourceBlock_.workspace;
        this.getValue() ? this.uniqueName_ ? this.sourceBlock_.isInFlyout || this.setValue(Blockly.Instances.convertToUniqueNameBlock(this.getValue(), this.sourceBlock_)) : (a = Blockly.Instances.getAnyInstanceOf(this.instanceType_, a)) && this.setValue(a) : this.setValue(Blockly.Instances.generateUniqueName(a))
    }
};
Blockly.FieldInstance.prototype.getValue = function() {
    return this.getText()
};
Blockly.FieldInstance.prototype.getInstanceTypeValue = function(a) {
    return a === this.instanceType_ ? this.getText() : void 0
};
Blockly.FieldInstance.prototype.setValue = function(a) {
    this.sourceBlock_ && Blockly.Events.isEnabled() && Blockly.Events.fire(new Blockly.Events.Change(this.sourceBlock_, "field", this.name, this.value_, a));
    this.value_ = a;
    this.setText(a)
};
Blockly.FieldInstance.prototype.dropdownCreate = function() {
    var a = this.sourceBlock_ && this.sourceBlock_.workspace ? Blockly.Instances.allInstancesOf(this.instanceType_, this.sourceBlock_.workspace) : [],
        b = this.getText();
    b && -1 == a.indexOf(b) && a.push(b);
    a.sort(goog.string.caseInsensitiveCompare);
    this.lockRename_ || a.push(Blockly.Msg.RENAME_INSTANCE);
    this.lockNew_ || a.push(Blockly.Msg.NEW_INSTANCE);
    for (var c = this.editDropdownData ? this.editDropdownData(c) : [], b = 0; b < a.length; b++) c[b] = [a[b], a[b]];
    return c
};
Blockly.FieldInstance.dropdownChange = function(a) {
    function b(a, b, c) {
        Blockly.hideChaff();
        Blockly.prompt(a, b, function(a) {
            a && (a = a.replace(/[\s\xa0]+/g, " ").replace(/^ | $/g, ""), a == Blockly.Msg.RENAME_INSTANCE || a == Blockly.Msg.NEW_INSTANCE) && (a = null);
            c(a)
        })
    }
    var c = this.sourceBlock_.workspace;
    if (a == Blockly.Msg.RENAME_INSTANCE) {
        var d = this.getText(),
            e = this;
        b(Blockly.Msg.RENAME_INSTANCE_TITLE.replace("%1", d), d, function(a) {
            a && Blockly.Instances.renameInstance(d, a, e.instanceType_, c)
        });
        return null
    }
    if (a == Blockly.Msg.NEW_INSTANCE) return Blockly.Instances.generateUniqueName(c)
};
...
Blockly.FieldNumber = function(a, b) {
    Blockly.FieldNumber.superClass_.constructor.call(this, a, b)
};
...
Blockly.Variables.allVariables = function(a) {
    var b;
    if (a.getDescendants) b = a.getDescendants();
...
Blockly.Variables.renameVariable = function(a, b, c) {
    Blockly.Events.setGroup(!0);
    c = c.getAllBlocks();
    for (var d = 0; d < c.length; d++) c[d].renameVar(a, b);
    Blockly.Events.setGroup(!1)
};
Blockly.Variables.flyoutCategory = function(a) {
    a = Blockly.Variables.allVariables(a);
    a.sort(goog.string.caseInsensitiveCompare);
    goog.array.remove(a, Blockly.Msg.VARIABLES_DEFAULT_NAME);
    a.unshift(Blockly.Msg.VARIABLES_DEFAULT_NAME);
    for (var b = [], c = 0; c < a.length; c++) {
        if (Blockly.Blocks.variables_set) {
            var d = goog.dom.createDom("block");
            d.setAttribute("type", "variables_set");
            Blockly.Blocks.variables_get && d.setAttribute("gap", 8);
            var e = goog.dom.createDom("field", null, a[c]);
            e.setAttribute("name", "VAR");
            d.appendChild(e);
            b.push(d)
        }
        Blockly.Blocks.variables_get && (d = goog.dom.createDom("block"), d.setAttribute("type", "variables_get"), Blockly.Blocks.variables_set && d.setAttribute("gap", 24), e = goog.dom.createDom("field", null, a[c]), e.setAttribute("name", "VAR"), d.appendChild(e), b.push(d))
    }
    return b
};
...
Blockly.FieldVariable = function(a, b) {
    Blockly.FieldVariable.superClass_.constructor.call(this, this.dropdownCreate, b);
    this.setValue(a || "")
};
goog.inherits(Blockly.FieldVariable, Blockly.FieldDropdown);
Blockly.FieldVariable.prototype.setValidator = function(a) {
    Blockly.FieldVariable.superClass_.setValidator.call(this, a ? function(b) {
        var c = a.call(this, b);
        if (null === c) var d = c;
...
Blockly.FieldVariable.prototype.init = function() {
    this.fieldGroup_ || (Blockly.FieldVariable.superClass_.init.call(this), this.getValue() || this.setValue(Blockly.Variables.generateUniqueName(this.sourceBlock_.isInFlyout ? this.sourceBlock_.workspace.targetWorkspace : this.sourceBlock_.workspace)))
};
...
Blockly.FieldVariable.dropdownChange = function(a) {
    ...
    if (a == Blockly.Msg.RENAME_VARIABLE) {
        var d = this.getText();
        b(Blockly.Msg.RENAME_VARIABLE_TITLE.replace("%1", d), d, function(a) {
            a && Blockly.Variables.renameVariable(d, a, c)
        });
        return null
    }
    if (a == Blockly.Msg.NEW_VARIABLE) return Blockly.Variables.generateUniqueName(c)
};
...
Blockly.Generator.prototype.blockToCode = function(a) {
    ...
    if (goog.isString(b)) return this.STATEMENT_PREFIX && (b = this.STATEMENT_PREFIX.replace(/%1/g,
        "'" + a.id + "'") + b), this.scrub_(a, b);
    if (null === b) return "";
...
Blockly.StaticTyping.getAllStatementsOrdered = function(a) {
    ...
    var b = function(a) {
            ...
            do {
                ...
                d = a.getConnections_();
                a = null;
                for (var l = 0; l < d.length; l++) d[l].type == Blockly.NEXT_STATEMENT && (c = d[l].targetBlock()) && (d[l] === e ? a = c : k = k.concat(b(c)))
            } while (a);
            return k
        },
        c = [];
...
Blockly.Procedures.flyoutCategory = function(a) {
    function b(a, b) {
        for (var d = 0; d < a.length; d++) {
            ...
            var h = goog.dom.createDom("mutation");
            h.setAttribute("name", e);
            g.appendChild(h);
            for (e = 0; e < f.length; e++) {
                var n = goog.dom.createDom("arg");
                n.setAttribute("name", f[e]);
                h.appendChild(n)
            }
            c.push(g)
        }
    ...
    if (Blockly.Blocks.arduino_functions) {
        var d = goog.dom.createDom("block");
        d.setAttribute("type", "arduino_functions");
        d.setAttribute("gap", 16);
        for (var e = a.getTopBlocks(), f = 0; f < e.length; f++) e[f].getArduinoLoopsInstance && e[f].getArduinoLoopsInstance() && d.setAttribute("disabled", !0);
        c.push(d)
    }
    Blockly.Blocks.procedures_defnoreturn && (d = goog.dom.createDom("block"), d.setAttribute("type", "procedures_defnoreturn"), d.setAttribute("gap", 16), c.push(d));
    ...
    Blockly.Blocks.procedures_ifreturn &&
        (d = goog.dom.createDom("block"), d.setAttribute("type", "procedures_ifreturn"), d.setAttribute("gap", 16), c.push(d));
...
Blockly.Procedures.mutateCallers = function(a) {
    ...
    for (var c = 0, e; e = a[c]; c++) {
        ...
        f != g && (Blockly.Events.recordUndo = !1, Blockly.Events.fire(new Blockly.Events.Change(e, "mutation", null, f, g)), Blockly.Events.recordUndo = b)
    }
};
...
Blockly.Flyout.prototype.getMetrics_ = function() {
    ...
    if (this.horizontalLayout_) {
        ...
        var d = this.height_;
        this.toolboxPosition_ == Blockly.TOOLBOX_AT_TOP && (d += this.MARGIN - this.SCROLLBAR_PADDING);
        var e = this.width_ - 2 * this.SCROLLBAR_PADDING
    } else c = 0, d = this.height_ - 2 * this.SCROLLBAR_PADDING,
        e = this.width_, this.RTL || (e -= this.SCROLLBAR_PADDING);
    return {
        viewHeight: d,
        viewWidth: e,
...
Blockly.Flyout.prototype.setMetrics_ = function(a) {
    ...
    b && (!this.horizontalLayout_ && goog.isNumber(a.y) ? this.workspace_.scrollY = -b.contentHeight * a.y : this.horizontalLayout_ && goog.isNumber(a.x) && (this.workspace_.scrollX = -b.contentWidth * a.x), this.workspace_.translate(this.workspace_.scrollX + b.absoluteLeft, this.workspace_.scrollY + b.absoluteTop))
};
Blockly.Flyout.prototype.position = function() {
    if (this.isVisible()) {
        var a = this.targetWorkspace_.getMetrics();
        if (a) {
            var b = this.horizontalLayout_ ? a.viewWidth : this.width_,
                b = b - this.CORNER_RADIUS;
            this.toolboxPosition_ == Blockly.TOOLBOX_AT_RIGHT && (b *= -1);
            this.setBackgroundPath_(b, this.horizontalLayout_ ? this.height_ : a.viewHeight);
            b = a.absoluteLeft;
            this.toolboxPosition_ == Blockly.TOOLBOX_AT_RIGHT && (b += a.viewWidth, b -= this.width_);
            var c = a.absoluteTop;
...
Blockly.Flyout.prototype.setBackgroundPathHorizontal_ = function(a, b) {
    ...
    c ? (d.push("h", a + this.CORNER_RADIUS), d.push("v", b), d.push("a", this.CORNER_RADIUS, this.CORNER_RADIUS, 0, 0, 1, -this.CORNER_RADIUS, this.CORNER_RADIUS), d.push("h", -1 * (a - this.CORNER_RADIUS)), d.push("a", this.CORNER_RADIUS, this.CORNER_RADIUS, 0, 0, 1, -this.CORNER_RADIUS, -this.CORNER_RADIUS)) : (d.push("a", this.CORNER_RADIUS, this.CORNER_RADIUS, 0, 0, 1, this.CORNER_RADIUS,
        -this.CORNER_RADIUS), d.push("h", a - this.CORNER_RADIUS), d.push("a", this.CORNER_RADIUS, this.CORNER_RADIUS, 0, 0, 1, this.CORNER_RADIUS, this.CORNER_RADIUS), d.push("v", b - this.CORNER_RADIUS), d.push("h", -a - this.CORNER_RADIUS));
    d.push("z");
...
Blockly.Flyout.prototype.show = function(a) {
    ...
    for (var b = [], c = [], d = this.permanentlyDisabled_.length = 0, e; e = a[d]; d++)
        if (e.tagName && "BLOCK" == e.tagName.toUpperCase()) {
            var f = Blockly.Xml.domToBlock(e, this.workspace_);
            f.disabled && this.permanentlyDisabled_.push(f);
            b.push(f);
            e = parseInt(e.getAttribute("gap"), 10);
            c.push(isNaN(e) ? 3 * this.MARGIN : e)
        } this.layoutBlocks_(b, c);
    ...
    this.reflow();
    this.offsetHorizontalRtlBlocks(this.workspace_.getTopBlocks(!1));
    this.filterForCapacity_();
    Blockly.svgResize(this.workspace_);
    this.reflowWrapper_ = this.reflow.bind(this);
    this.workspace_.addChangeListener(this.reflowWrapper_)
};
Blockly.Flyout.prototype.layoutBlocks_ = function(a, b) {
    for (var c = this.MARGIN * this.workspace_.scale, d = this.RTL ? c : c + Blockly.BlockSvg.TAB_WIDTH, e = 0, f; f = a[e]; e++) {
        ...
        f.moveBy(this.horizontalLayout_ && this.RTL ? -d : d, c);
...
Blockly.Flyout.prototype.clearOldBlocks_ = function() {
    ...
    for (a = 0; b = this.buttons_[a]; a++) goog.dom.removeNode(b);
...
Blockly.Flyout.prototype.onMouseDown_ = function(a) {
    Blockly.isRightButton(a) || (Blockly.hideChaff(!0), Blockly.Flyout.terminateDrag_(), this.startDragMouseY_ = a.clientY, this.startDragMouseX_ = a.clientX, Blockly.Flyout.onMouseMoveWrapper_ = Blockly.bindEvent_(document, "mousemove", this, this.onMouseMove_), Blockly.Flyout.onMouseUpWrapper_ = Blockly.bindEvent_(document, "mouseup", this, Blockly.Flyout.terminateDrag_), a.preventDefault(), a.stopPropagation())
};
Blockly.Flyout.prototype.onMouseUp_ = function(a) {
    Blockly.dragMode_ == Blockly.DRAG_FREE || Blockly.WidgetDiv.isVisible() || Blockly.Events.fire(new Blockly.Events.Ui(Blockly.Flyout.startBlock_, "click", void 0, void 0));
    Blockly.terminateDrag_()
};
Blockly.Flyout.prototype.onMouseMove_ = function(a) {
    ...
    } else 0 > b.contentHeight - b.viewHeight || (c = a.clientY - this.startDragMouseY_, this.startDragMouseY_ = a.clientY, a = b.viewTop - c, a = goog.math.clamp(a, 0, b.contentHeight - b.viewHeight), this.scrollbar_.set(a))
};
...
Blockly.Flyout.prototype.createBlockFunc_ = function(a) {
    ...
    return function(c) {
        if (!Blockly.isRightButton(c) && !a.disabled) {
            ...
            Blockly.Events.enable();
            Blockly.Events.isEnabled() && (Blockly.Events.setGroup(!0), Blockly.Events.fire(new Blockly.Events.Create(d)));
...
Blockly.Flyout.prototype.placeNewBlock_ = function(a) {
    ...
    var c = Blockly.getSvgXY_(c, b),
        d = this.workspace_.scrollX,
        e = this.workspace_.scale;
    c.x += d / e - d;
    this.toolboxPosition_ == Blockly.TOOLBOX_AT_RIGHT && (d = b.getMetrics().viewWidth - this.width_, e = b.scale, c.x += d / e - d);
    d = this.workspace_.scrollY;
    e = this.workspace_.scale;
    c.y += d / e - d;
    this.toolboxPosition_ == Blockly.TOOLBOX_AT_BOTTOM && (d = b.getMetrics().viewHeight - this.height_, e = b.scale,
        c.y += d / e - d);
    ...
    b.toolbox_ && !b.scrollbar && (e.x += b.toolbox_.getWidth() / b.scale, e.y += b.toolbox_.getHeight() / b.scale);
...
Blockly.Flyout.prototype.reflowHorizontal = function(a) {
    ...
    for (var b = 0, c = 0, d; d = a[c]; c++) b = Math.max(b, d.getHeightWidth().height);
    b += 1.5 * this.MARGIN;
        ...
        for (c = 0; d = a[c]; c++) {
            ...
            if (this.RTL) {
                var f = d.getRelativeToSurfaceXY().x,
                    g = b - this.MARGIN,
                    g = g / this.workspace_.scale,
                    g = g - Blockly.BlockSvg.TAB_WIDTH;
                d.moveBy(g - f, 0)
            }
            d.flyoutRect_ && (d.flyoutRect_.setAttribute("width", e.width), d.flyoutRect_.setAttribute("height", e.height), g = d.outputConnection ? Blockly.BlockSvg.TAB_WIDTH : 0, f = d.getRelativeToSurfaceXY(), d.flyoutRect_.setAttribute("x", this.RTL ? f.x - e.width + g : f.x - g), (e = d.startHat_ ? Blockly.BlockSvg.START_HAT_HEIGHT : 0) && d.moveBy(0, e), d.flyoutRect_.setAttribute("y", f.y))
        }
        this.width_ = b;
        Blockly.asyncSvgResize(this.workspace_)
    }
...
Blockly.Flyout.prototype.offsetHorizontalRtlBlocks = function(a) {
    if (this.horizontalLayout_ && this.RTL) {
        this.position();
        try {
            var b = this.workspace_.getCanvas().getBBox()
        } catch (e) {
            b = {
                height: 0,
                y: 0,
        ...
        for (var b = Math.max(-b.x + this.MARGIN, this.width_ / this.workspace_.scale), c = 0, d; d = a[c]; c++) d.moveBy(b, 0), d.flyoutRect_ && d.flyoutRect_.setAttribute("x", b + Number(d.flyoutRect_.getAttribute("x")))
    }
};
...
Blockly.Toolbox.prototype.position = function() {
    ...
    if (a) {
        var b = this.workspace_.getParentSvg(),
            ...
            b = Blockly.svgSize(b);
        this.horizontalLayout_ ? (a.style.left = c.x + "px", a.style.height = "auto", a.style.width = b.width + "px", this.height = a.offsetHeight, a.style.top = this.toolboxPosition == Blockly.TOOLBOX_AT_TOP ? c.y + "px" : c.y + b.height - a.offsetHeight + "px") : (a.style.left = this.toolboxPosition == Blockly.TOOLBOX_AT_RIGHT ? c.x + b.width - a.offsetWidth + "px" : c.x + "px", a.style.height = b.height +
            "px", a.style.top = c.y + "px", this.width = a.offsetWidth, this.toolboxPosition == Blockly.TOOLBOX_AT_LEFT && --this.width);
        this.flyout_.position()
    }
};
Blockly.Toolbox.prototype.populate_ = function(a) {
    function b(a, g, h) {
        for (var f = null, l = 0, p; p = a.childNodes[l]; l++)
            if (p.tagName) switch (p.tagName.toUpperCase()) {
                case "CATEGORY":
                    f = c.createNode(p.getAttribute("name"));
                    f.blocks = [];
                    g.add(f);
                    var m = p.getAttribute("custom");
                    m ? f.blocks = m : b(p, f, h);
                    m = p.getAttribute("colour");
                    goog.isString(m) ? (m.match(/^#[0-9a-fA-F]{6}$/) ? f.hexColour = m : f.hexColour = Blockly.hueToRgb(m), e = !0) : f.hexColour = "";
                    "true" == p.getAttribute("expanded") ? (f.blocks.length && c.setSelectedItem(f), f.setExpanded(!0)) :
                        f.setExpanded(!1);
                    f = p;
                    break;
                case "SEP":
                    f && ("CATEGORY" == f.tagName.toUpperCase() ? g.add(new Blockly.Toolbox.TreeSeparator(d.treeSeparatorConfig_)) : (p = parseFloat(p.getAttribute("gap")), isNaN(p) || (m = parseFloat(f.getAttribute("gap")), p = isNaN(m) ? p : m + p, f.setAttribute("gap", p))));
                    break;
                case "BLOCK":
                case "SHADOW":
                    g.blocks.push(p), f = p
    ...
    c.blocks = [];
    var e = !1;
    b(a, this.tree_, this.workspace_.options.pathToMedia);
    this.hasColours_ = e;
...
Blockly.Toolbox.TreeNode = function(a, b, c, d) {
    ...
    a && (b = function() {
        Blockly.asyncSvgResize(a.workspace_)
...
Blockly.Css.inject = function(a, b) {
    if (!Blockly.Css.styleSheet_) {
        ...
        var c = c.replace(/<<<PATH>>>/g, Blockly.Css.mediaPath_),
...
Blockly.Css.setCursor = function(a) {
    if (Blockly.Css.currentCursor_ != a) {
        ...
        for (var c = document.getElementsByClassName("blocklyToolboxDiv"), d = 0, e; e = c[d]; d++) e.style.cursor = a == Blockly.Css.Cursor.DELETE ? b : "";
...
Blockly.Css.CONTENT = [".blocklySvg {", "background-color: #fff;", "outline: none;", "overflow: hidden;", "}", ".blocklyWidgetDiv {", "display: none;", "position: absolute;", "z-index: 999;", "}", ".blocklyTooltipDiv {", "background-color: #ffffc7;", "border: 1px solid #ddc;", "box-shadow: 4px 4px 20px 1px rgba(0,0,0,.15);", "color: #000;", "display: none;", "font-family: sans-serif;", "font-size: 9pt;", "opacity: 0.9;", "padding: 2px;", "position: absolute;", "z-index: 1000;", "}", ".blocklyResizeSE {", "cursor: se-resize;",
    ...
    "font-size: 16px;", "padding: 0 3px;", "vertical-align: middle;", "}", ".blocklyTreeSelected .blocklyTreeLabel {", "color: #fff;", "}", ".blocklyWidgetDiv .goog-palette {", "outline: none;", "cursor: default;", "}", ".blocklyWidgetDiv .goog-palette-table {", "border: 1px solid #666;", "border-collapse: collapse;", "}", ".blocklyWidgetDiv .goog-palette-cell {", "height: 13px;", "width: 15px;", "margin: 0;", "border: 0;", "text-align: center;", "vertical-align: middle;", "border-right: 1px solid #666;", "font-size: 1px;",
    "}", ".blocklyWidgetDiv .goog-palette-colorswatch {", "position: relative;", "height: 13px;", "width: 15px;", "border: 1px solid #666;", "}", ".blocklyWidgetDiv .goog-palette-cell-hover .goog-palette-colorswatch {", "border: 1px solid #FFF;", "}", ".blocklyWidgetDiv .goog-palette-cell-selected .goog-palette-colorswatch {", "border: 1px solid #000;", "color: #fff;", "}", ".blocklyWidgetDiv .goog-menu {", "background: #fff;", "border-color: #ccc #666 #666 #ccc;", "border-style: solid;", "border-width: 1px;", "cursor: default;",
...
Blockly.createMainWorkspace_ = function(a, b) {
    ...
    b.readOnly || b.hasScrollbars || c.addChangeListener(function() {
        if (Blockly.dragMode_ == Blockly.DRAG_NONE) {
            ...
            if (a.contentTop <
                f || a.contentTop + a.contentHeight > a.viewHeight + f || a.contentLeft < (b.RTL ? a.viewLeft : e) || a.contentLeft + a.contentWidth > (b.RTL ? a.viewWidth : a.viewWidth + e))
                for (var g = c.getTopBlocks(!1), h = 0, k; k = g[h]; h++) {
                    ...
                    0 < m && k.moveBy(m, 0);
                    l = e + a.viewWidth - 25 - l.x + (b.RTL ? p.width : 0);
                    0 > l && k.moveBy(l, 0)
                }
        }
    });
...
Blockly.init_ = function(a) {
    ...
    Blockly.bindEvent_(window, "resize", null, function() {
        Blockly.hideChaff(!0);
        Blockly.asyncSvgResize(a)
    });
    Blockly.inject.bindDocumentEvents_();
    b.languageTree && (a.toolbox_ ? a.toolbox_.init(a) : a.flyout_ && (a.flyout_.init(a), a.flyout_.show(b.languageTree.childNodes), a.flyout_.scrollToStart(), a.scrollX = a.flyout_.width_, b.toolboxPosition == Blockly.TOOLBOX_AT_RIGHT && (a.scrollX *=
        -1), a.translate(a.scrollX, 0)));
...
Blockly.inject.loadSounds_ = function(a, b) {
    ...
    var c = [],
        d = function() {
            for (; c.length;) Blockly.unbindEvent_(c.pop());
            b.preloadAudio_()
        };
    c.push(Blockly.bindEvent_(document, "mousemove", null, d));
    c.push(Blockly.bindEvent_(document, "touchstart", null, d))
};
...
Blockly.svgResizePending_ = !1;
Blockly.svgResize = function(a) {
    for (Blockly.svgResizePending_ = !1; a.options.parentWorkspace;) a = a.options.parentWorkspace;
...
Blockly.onMouseMove_ = function(a) {
    if (!(a.touches && 2 <= a.touches.length)) {
        ...
        if (b.isScrolling) {
            ...
            Math.sqrt(c * c + d * d) > Blockly.DRAG_RADIUS &&
                Blockly.longStop_();
            a.stopPropagation();
            a.preventDefault()
...
Blockly.onKeyDown_ = function(a) {
    if (!Blockly.mainWorkspace.options.readOnly && !Blockly.isTargetInput_(a)) {
        ...
        else if (8 == a.keyCode || 46 == a.keyCode) a.preventDefault(), Blockly.selected && Blockly.selected.isDeletable() && (b = !0);
        else if (a.altKey || a.ctrlKey || a.metaKey) Blockly.selected && Blockly.selected.isDeletable() && Blockly.selected.isMovable() && (67 == a.keyCode ? (Blockly.hideChaff(), Blockly.copy_(Blockly.selected)) : 88 == a.keyCode && (Blockly.copy_(Blockly.selected), b = !0)),
            86 == a.keyCode ? Blockly.clipboardXml_ && Blockly.clipboardSource_.paste(Blockly.clipboardXml_) : 90 == a.keyCode && (Blockly.hideChaff(), Blockly.mainWorkspace.undo(a.shiftKey));
        b && (Blockly.Events.setGroup(!0), Blockly.hideChaff(), Blockly.selected.dispose(Blockly.dragMode_ != Blockly.DRAG_FREE, !0), Blockly.highlightedConnection_ && (Blockly.highlightedConnection_.unhighlight(), Blockly.highlightedConnection_ = null), Blockly.Events.setGroup(!1))
    }
};
...
Blockly.getMainWorkspaceMetrics_ = function() {
    ...
    if (this.toolbox_)
        if (this.toolboxPosition == Blockly.TOOLBOX_AT_TOP || this.toolboxPosition == Blockly.TOOLBOX_AT_BOTTOM) a.height -= this.toolbox_.getHeight();
        else if (this.toolboxPosition == Blockly.TOOLBOX_AT_LEFT || this.toolboxPosition == Blockly.TOOLBOX_AT_RIGHT) a.width -= this.toolbox_.getWidth();
    var b = Blockly.Flyout.prototype.CORNER_RADIUS - 1,
        ...
        h = e.x * this.scale,
        k = e.y * this.scale;
    this.scrollbar ? (b = Math.min(h - c / 2, h + f - c), c = Math.max(h + f + c / 2, h + c), f = Math.min(k - d / 2, k + g - d), d = Math.max(k + g + d / 2, k + d)) : (b = e.x, c = b + e.width, f = e.y, d = f + e.height);
    e = 0;
